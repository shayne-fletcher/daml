...
class SupervisingActor(context: ActorContext[String]) extends AbstractBehavior[String](context) {
  private val child = context.spawn(Behaviors.supervise(SupervisedActor()).onFailure(restart),
...
object Main3 {
  def apply(): Behavior[String] =
    Behaviors.setup(context => new Main3(context))
}

class Main3(context: ActorContext[String]) extends AbstractBehavior[String](context) {
  override def onMessage(msg: String): Behavior[String] =
    msg match {
      case "start" =>
...
object ActorFailureHandlingExperiments extends App {
  val testSystem = ActorSystem(Main3(), "testSystem")
  testSystem ! "start"
}
